...
('Mouse trap', 1), ('Mustang', 1), ('The Lift', 1), ('The Sun', 1), ('The lift', 1)]
    ...
    films = [(film_name, len(find_film_keywords(film_keywords, film_name)))
             for film_name in {film for film_list in film_keywords.values() for film in film_list}]

    return sorted(films, key=lambda x: (-x[1], x[0]))[:num_of_films]
